...
public class KaraokeSessionStatics : InMemoryConfigManager<KaraokeRulesetSession>
{
    ...
    public KaraokeSessionStatics(KaraokeRulesetConfigManager config, IBeatmap? beatmap)
    {
        ...
        // Live Uta pitch error, expressed in semitones from the authored target.
        SetDefault(KaraokeRulesetSession.PitchDeviation, 0f, -12f, 12f);
        SetDefault(KaraokeRulesetSession.DetectedPitchMidi, 60f, 0f, 127f);
        SetDefault(KaraokeRulesetSession.PitchSimilarity, 0f, 0f, 1f);
        SetDefault(KaraokeRulesetSession.VoiceActive, false);
    }

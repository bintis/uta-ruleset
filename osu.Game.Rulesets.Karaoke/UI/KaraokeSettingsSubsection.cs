...
public partial class KaraokeSettingsSubsection : RulesetSettingsSubsection
{
    ...
    private void load(OsuGame game, IPerformFromScreenRunner performer)
    {
        ...
        Children = new Drawable[]
        {
            ...
            {
                LabelText = KaraokeSettingsSubsectionStrings.MicrophoneMonitorVolume,
                TooltipText = KaraokeSettingsSubsectionStrings.MicrophoneMonitorVolumeTooltip,
                Current = config.GetBindable<float>(KaraokeRulesetSetting.MicrophoneMonitorVolume),
                KeyboardStep = 0.05f,
                DisplayAsPercentage = true,
            },
            new SettingsSlider<float>
            {
                LabelText = "Guide voice volume",
                TooltipText = "Volume of the guide vocal track embedded in a UTZ song.",
                Current = config.GetBindable<float>(KaraokeRulesetSetting.GuideVoiceVolume),
                KeyboardStep = 0.05f,
                DisplayAsPercentage = true,
            },
            // Practice

...
public partial class KaraokeRuleset : Ruleset
{
    ...
    public override HealthProcessor CreateHealthProcessor(double drainStartTime) => new KaraokeHealthProcessor(drainStartTime);
    ...
    public override IRulesetFilterCriteria CreateRulesetFilterCriteria() => new KaraokeFilterCriteria();
    ...
    {
        string? key = UtaKeySignature.FromMetadataTags(beatmapInfo.Metadata.Tags);
        if (string.IsNullOrWhiteSpace(key))
            yield break;

        // The native attribute card expects a numeric value. A quoted custom
        // numeric format lets it render the authored musical key verbatim while
        // retaining lazer's standard song-select presentation.
        yield return new RulesetBeatmapAttribute("Key signature", "KEY", 1, 1, 1)
        {
            ValueFormat = $"'{key.Replace("'", string.Empty)}'",
        };
    }
    ...
    public override IEnumerable<Mod> GetModsFor(ModType type) =>
        type switch
        {
            ...
            {
                new KaraokeModCanFail(),
                new KaraokeModHidePitchGuide(),
                new KaraokeModHideLyrics(),
            },
            ModType.DifficultyReduction => new Mod[]
            {
                new KaraokeModOriginalVocals(),
            },
    ...
    private partial class KaraokeIcon : CompositeDrawable
    {
        ...
        private void load(IRenderer renderer, OsuGameBase? game, BeatmapManager? beatmapManager, INotificationOverlay? notifications)
        {
            UtaBassPluginLoader.EnsureLoaded();

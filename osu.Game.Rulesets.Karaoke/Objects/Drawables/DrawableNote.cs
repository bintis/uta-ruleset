...
public partial class DrawableNote : DrawableKaraokeScrollingHitObject<Note>, IKeyBindingHandler<KaraokeScoringAction>
{
    ...
    private readonly UtaPitchScoring utaScoring = new();
    private float? utaVoiceHertz;
    private bool utaScoringStarted;
    private double lastUtaScoringTime;
    private readonly BindableFloat livePitchHertz = new();
    private readonly BindableBool liveVoiceActive = new();
    private bool hasLiveUtaInput;
    ...
    private void load(INotePositionInfo notePositionInfo, DrawableRuleset? drawableRuleset)
    {
        if (drawableRuleset is DrawableKaraokeRuleset karaokeRuleset)
        {
            livePitchHertz.BindTo(karaokeRuleset.KeyBindingInputManager.LivePitchHertz);
            liveVoiceActive.BindTo(karaokeRuleset.KeyBindingInputManager.LiveVoiceActive);
            hasLiveUtaInput = true;
        }
    ...
    {
        if (HitObject.Midi is { } midi && !Result.HasResult && Time.Current >= HitObject.StartTime)
        {
            if (hasLiveUtaInput)
            {
                utaVoiceHertz = liveVoiceActive.Value && livePitchHertz.Value > 0 ? livePitchHertz.Value : null;
                if (utaVoiceHertz != null && holdStartTime == null && Time.Current <= HitObject.EndTime)
                    BeginSing();
            }

            double scoringTime = Math.Min(Time.Current, HitObject.EndTime) / 1000;
            if (!utaScoringStarted)
            {
                utaScoring.Reset();
                utaScoring.Discontinuity(HitObject.StartTime / 1000);
                lastUtaScoringTime = HitObject.StartTime / 1000;
                utaScoringStarted = true;
            }

            if (scoringTime > lastUtaScoringTime)
            {
                double targetHertz = UtaPitchMath.MidiToFrequency(midi);
                utaScoring.AddFrame(
                    scoringTime,
                    targetHertz,
                    utaVoiceHertz,
                    ...
                    0,
                    midi,
                    HitObject.UtaOctaveTolerance);
                lastUtaScoringTime = scoringTime;
            }
        }
    ...
    protected override void CheckForResult(bool userTriggered, double timeOffset)
    {
        ...
        if (HitObject.Midi != null)
        {
            if (userTriggered || Time.Current < HitObject.EndTime)
                return;

            stopUtaSing();
            var noteScore = utaScoring.GetNoteScores();
            ApplyResult(noteScore.Count > 0 ? noteScore[0].HitResult : HitResult.Miss);
            return;
        }
    ...
    public bool OnPressed(KeyBindingPressEvent<KaraokeScoringAction> e)
    {
        ...
        if (HitObject.Midi != null)
        {
            utaVoiceHertz = e.Action.Hertz > 0 ? e.Action.Hertz : null;
    ...
    public void OnReleased(KeyBindingReleaseEvent<KaraokeScoringAction> e)
    {
        ...
        // Uta notes are judged over their complete authored duration rather than
        // when the microphone happens to cross the silence gate.
        if (HitObject.Midi != null)
            stopUtaSing();
        else
            EndSing();
    ...
    {
        livePitchHertz.UnbindAll();
        liveVoiceActive.UnbindAll();

...
public sealed partial class UtzPackage
{
    ...
    public static UtzPackage Open(Stream stream)
    {
        ...
        try
        {
            manifest = JsonConvert.DeserializeObject<UtzManifest>(Encoding.UTF8.GetString(readEntry(manifestEntry)), jsonSettings)
    ...
    private T parseJson<T>(UtzAsset asset, string role)
    {
        try
        {
            return JsonConvert.DeserializeObject<T>(Encoding.UTF8.GetString(GetAsset(asset).Span), jsonSettings)

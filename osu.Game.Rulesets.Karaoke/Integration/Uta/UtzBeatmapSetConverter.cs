...
public static class UtzBeatmapSetConverter
{
    ...
    public static void Convert(Stream input, Stream output)
    {
        ...
        addText(archive, "manifest.json", JsonConvert.SerializeObject(package.Manifest, Formatting.Indented, jsonSettings));
    ...
    private static string createBeatmap(UtzPackage package)
    {
        ...
        string? keyTag = UtaKeySignature.CreateMetadataTag(manifest.Song.Key);
        ...
        var lines = new List<string>
        {
            ...
            $"Tags:uta utz karaoke{(keyTag == null ? string.Empty : $" {keyTag}")}",
            "BeatmapID:0",
            "BeatmapSetID:-1",
            string.Empty,
            "[Difficulty]",
        ...
        lines.AddRange(segments.Select(segment => $"@utalyric={encode(segment)}"));
    ...
    private static readonly JsonSerializerSettings jsonSettings = new()
    {
        ...
        Converters = { new StringEnumConverter(new Newtonsoft.Json.Serialization.SnakeCaseNamingStrategy()) },
    };
}

use crate::bin::{encode_command, encode_trace, parse as parse_bin, KIND_ERROR, KIND_QUEUE, KIND_RESULT, KIND_RESUMED, KIND_STATE, KIND_WELCOME};
use crate::i18n::{t, Lang};
use crate::proto::{parse_envelope, LibrarySong, QueueEntry, Snapshot};
use crate::{clip, fill_rect, fill_text, log, measure, search_rect, send, set_remembered, store_seq, store_session, stroke_rect, unclip};
...
impl App {
    ...
    pub fn load_demo(&mut self) {
        ...
        self.queue = (0..8)
            .map(|i| QueueEntry {
                ...
                mods: if i == 3 { vec!["NF".into(), "IQ".into()] } else { Vec::new() },
            })
            .collect();
            ...
            match msg.kind {
                KIND_WELCOME | KIND_RESUMED => {
                    ...
                    log(&format!(
                        ...
                        if msg.kind == KIND_WELCOME { "welcome" } else { "resumed" },
                        self.role,
    ...
    pub fn pointer(&mut self, kind: i32, x: f32, y: f32) {
        ...
        match kind {
            ...
            1 if self.pointer_down => {
                ...
                match self.drag {
                    DragKind::Pager => {
                        let next = (self.start_page - dx / self.width).clamp(0.0, (PAGE_COUNT - 1) as f32);
                        self.page = next;
                    }
                    DragKind::Scroll => {
                        self.push_vel(y);
                ...
                if self.pointer_down {
                    ...
                    match self.drag {
                        DragKind::Pager => {
                            ...
                            if dx < -threshold {
                                self.target = (self.start_page.round() as i32 + 1).min(PAGE_COUNT - 1);
    ...
    fn axis_free(&self) -> bool {
        !matches!(self.drag, DragKind::Seek | DragKind::Slider | DragKind::Reorder)
    }
    ...
    fn fire(&mut self, action: Action) {
        ...
        match action {
            ...
            Action::Octave => {
                self.cmd("octaveFold", None, Some(!self.snapshot.mixer.octave_fold), None);
            }
            Action::Vocals => {
                self.cmd(
                    "originalVocals",
    ...
    fn submit_sheet(&mut self, add_next: bool, play_now: bool) {
        ...
        match sheet.kind {
            SheetKind::Add => {
                if play_now {
                    self.play_after_add = false;
                    self.request("queuePlayNow", None, None, Some(&sheet.id), options, "playNow");
                    return;
                }
                self.play_after_add = false;
            ...
            SheetKind::Edit => {
                self.request("queueConfigure", None, None, Some(&sheet.id), options, "saved");
            }
        }
    }
    ...
    fn ingest_library(&mut self, page: Vec<LibrarySong>) {
        const PAGE: usize = 80;
        log(&format!("ui.recv library page={} offset={}", page.len(), self.library_offset));
        ...
        if self.library_offset == 0 {
            ...
            for song in page {
                if !self.library.iter().any(|existing| existing.beatmap_id == song.beatmap_id) {
    ...
    pub fn frame(&mut self, now: f64) {
        ...
        if let Some(toast) = &self.toast {
            ...
            fill_rect((self.width - w) * 0.5, self.height - 76.0 - self.inset_b, w, 38.0, PANEL2, 6.0);
            stroke_rect((self.width - w) * 0.5, self.height - 76.0 - self.inset_b, w, 38.0, color, 6.0, 1.0);
            fill_text(self.width * 0.5, self.height - 57.0 - self.inset_b, 14.0, color, 1, 1, &toast.text);
        }
    ...
    fn draw_chrome(&mut self) {
        ...
        fill_rect(12.0 + brand_w + 6.0, cy - 4.0, 7.0, 7.0, if self.connected { OK } else { DANGER }, 4.0);
        ...
        if page == PAGE_LIST {
            fill_text(self.width - 12.0, cy, 11.0, if self.search_open { PINK } else { TEXT }, 2, 1, t(self.lang, "searchBtn"));
            self.hit(self.width - 64.0, 4.0 + self.inset_t, 56.0, 28.0, Action::ToggleSearch);
        } else if page == PAGE_QUEUE && self.controller() {
            fill_text(self.width - 14.0, cy, 16.0, if self.queue_menu { PINK } else { TEXT }, 2, 1, "···");
            self.hit(self.width - 44.0, 4.0 + self.inset_t, 40.0, 28.0, Action::QueueMenu);
        }
        ...
        if self.queue_menu && page == PAGE_QUEUE && self.controller() {
            ...
            let auto = if self.snapshot.auto_advance_enabled { PINK } else { TEXT };
            fill_text(mx + mw * 0.5, my + 22.0, 13.0, auto, 1, 1, t(self.lang, "auto"));
            self.hit(mx, my, mw, 36.0, Action::AutoAdvance);
            fill_text(mx + mw * 0.5, my + 52.0, 13.0, DANGER, 1, 1, t(self.lang, "clear"));
    ...
    fn draw_list(&mut self, x: f32, y: f32, h: f32) {
        ...
        if self.search_open {
            ...
            fill_rect(self.search_box.0, self.search_box.1, self.search_box.2, self.search_box.3, 0x6A000000, 6.0);
            list_y = y + 48.0;
        } else {
            self.search_box = (0.0, 0.0, 0.0, 0.0);
        }
        ...
        if self.library.is_empty() {
            fill_text(x + self.width * 0.5, list_y + 80.0, 14.0, MUTED, 1, 0, t(self.lang, "noSongs"));
        } else {
            ...
            for (offset, song) in songs.iter().enumerate() {
                ...
                self.draw_song_row(x + pad, ry + 4.0, self.width - pad * 2.0, SONG_ROW - 8.0, song, true);
            }
    ...
    fn draw_song_row(&mut self, x: f32, y: f32, w: f32, h: f32, song: &LibrarySong, open: bool) {
        ...
        let title_w = w - 20.0;
        fill_text(x + 14.0, y + 20.0, 16.0, TEXT, 0, 1, &ellipsize(16.0, 1, &song.title, title_w));
        let meta = format!(
            "{}  ·  [{}]  ·  {}{}",
            empty(&song.artist),
        ...
        fill_text(x + 14.0, y + 42.0, 11.0, MUTED, 0, 0, &ellipsize(11.0, 0, &meta, title_w));
        if open {
    ...
    fn draw_control(&mut self, x: f32, y: f32, _h: f32) {
        ...
        let lyrics = if live {
            ...
            self.snapshot.notice.as_deref().filter(|s| !s.is_empty()).unwrap_or("")
        };
        ...
        let seek_y = y + 86.0;
        fill_text(x + pad, seek_y + 10.0, 11.0, MUTED, 0, 0, &fmt_ms(self.snapshot.song_time));
        fill_text(x + self.width - pad, seek_y + 10.0, 11.0, MUTED, 2, 0, &fmt_ms(self.snapshot.song_length));
        let sl = x + pad + 40.0;
        let sw = inner - 80.0;
        ...
        fill_rect(sl + sw * tpos.clamp(0.0, 1.0) - 5.0, seek_y + 3.0, 10.0, 12.0, TEXT, 5.0);
        if live && self.controller() {
            self.hit(sl - 8.0, seek_y - 6.0, sw + 16.0, 28.0, Action::SeekTo(self.snapshot.song_time));
        }

        let col = (inner - 10.0) / 3.0;
        let tr_y = seek_y + 28.0;
        let play_label = if self.snapshot.paused { t(self.lang, "play") } else { t(self.lang, "pause") };
        self.lazer_button(x + pad, tr_y, col, 42.0, play_label, Action::PlayPause, live, false);
        self.lazer_button(x + pad + col + 5.0, tr_y, col, 42.0, t(self.lang, "nextSong"), Action::NextSong, live, true);
        self.outline_button(x + pad + (col + 5.0) * 2.0, tr_y, col, 42.0, t(self.lang, "skip"), Action::Skip, live, DANGER);

        let key = format!("{:+}", self.snapshot.mixer.transpose);
        let speed = format!("{}%", (self.snapshot.speed * 100.0).round());
        let half = (inner - 8.0) * 0.5;
        let step_y = tr_y + 50.0;
        self.compact_stepper(x + pad, step_y, half, t(self.lang, "key"), &key, Action::KeyDelta(-1), Action::KeyReset, Action::KeyDelta(1), live);
        self.compact_stepper(x + pad + half + 8.0, step_y, half, t(self.lang, "std"), &speed, Action::SpeedDelta(-0.05), Action::SpeedReset, Action::SpeedDelta(0.05), live);

        let mut yy = step_y + 44.0;
        if self.has_practice() {
            self.lazer_button(x + pad, yy, col, 36.0, t(self.lang, "prev"), Action::PrevPhrase, live, false);
            self.lazer_button(x + pad + col + 5.0, yy, col, 36.0, t(self.lang, "retry"), Action::Retry, live, false);
            self.lazer_button(x + pad + (col + 5.0) * 2.0, yy, col, 36.0, t(self.lang, "next"), Action::NextPhrase, live, false);
            yy += 42.0;
            self.lazer_button(x + pad, yy, col, 32.0, t(self.lang, "loopA"), Action::LoopA, live, false);
            self.lazer_button(x + pad + col + 5.0, yy, col, 32.0, t(self.lang, "loopB"), Action::LoopB, live, false);
            self.lazer_button(x + pad + (col + 5.0) * 2.0, yy, col, 32.0, t(self.lang, "clearLoop"), Action::ClearLoop, live, false);
            yy += 38.0;
            self.lazer_button(x + pad, yy, inner, 32.0, t(self.lang, "loopPhrase"), Action::LoopPhrase, live, false);
            yy += 40.0;
        }
        self.lazer_button(x + pad, yy, inner, 34.0, t(self.lang, "restart"), Action::Restart, live, false);
    }
    ...
    fn draw_queue(&mut self, x: f32, y: f32, h: f32) {
        ...
        clip(x, list_y, self.width, list_h);
        if self.queue.is_empty() {
            fill_text(x + self.width * 0.5, list_y + 72.0, 14.0, MUTED, 1, 0, t(self.lang, "emptyQueue"));
            fill_text(x + self.width * 0.5, list_y + 98.0, 12.0, MUTED, 1, 0, t(self.lang, "reorder"));
        } else {
            let scroll = self.display_scroll(PAGE_QUEUE);
            let controller = self.controller();
            let queue = self.queue.clone();
            let from = self.reorder_from;
            let to = if from.is_some() { Some(self.reorder_target()) } else { None };
            for (index, entry) in queue.iter().enumerate() {
                ...
                self.draw_queue_row(x + pad, ry + 4.0, self.width - pad * 2.0, QUEUE_ROW - 8.0, index, entry, controller, false);
            }
            if let Some(index) = from {
                if let Some(entry) = queue.get(index) {
                    ...
                    self.draw_queue_row(x + pad, ry + 2.0, self.width - pad * 2.0, QUEUE_ROW - 4.0, index, entry, controller, true);
                }
            }
        }
        unclip();
    ...
    fn draw_queue_row(
        ...
        fill_text(x + 14.0, y + 16.0, 11.0, PINK, 0, 1, &format!("{:02}", index + 1));
        let text_w = if controller { w - 56.0 } else { w - 20.0 };
        fill_text(x + 40.0, y + 18.0, 15.0, TEXT, 0, 1, &ellipsize(15.0, 1, &entry.title, text_w));
        let meta = format!(
            "{}  ·  [{}]  ·  {}",
            empty(&entry.artist),
        ...
        fill_text(x + 40.0, y + 40.0, 11.0, MUTED, 0, 0, &ellipsize(11.0, 0, &meta, text_w));
        let key = if entry.transpose == 0 {
            "K0".into()
        } else {
            format!("K{:+}", entry.transpose)
        };
        let chips = format!(
            "{}  ·  {}%{}",
            key,
    ...
    fn draw_info(&mut self, x: f32, y: f32, h: f32) {
        ...
        let rows: [(&str, String); 8] = [
            (
                ...
                self.snapshot.song_title.clone().unwrap_or_else(|| "—".into()),
            ),
            (
                t(self.lang, "list"),
                format!(
            ...
            (
                ...
                if self.snapshot.phrase_index >= 0 {
                    format!("{} / {}", self.snapshot.phrase_index + 1, self.snapshot.phrase_count)
                } else {
                    "—".into()
                },
            ),
            (t(self.lang, "key"), format!("{:+}", self.snapshot.mixer.transpose)),
            (
                t(self.lang, "std"),
                format!(
                    "{}% · {} – {}",
                    (self.snapshot.speed * 100.0).round(),
                    self.snapshot.r#loop().a.map(fmt_ms).unwrap_or_else(|| "—".into()),
                    self.snapshot.r#loop().b.map(fmt_ms).unwrap_or_else(|| "—".into())
                ),
            ),
        ];
        let scroll = self.display_scroll(PAGE_INFO);
        clip(x, y, self.width, h);
        let mut yy = y + 16.0 - scroll;
        for (label, value) in rows {
            fill_text(x + pad, yy, 12.0, MUTED, 0, 0, label);
            fill_text(x + self.width - pad, yy, 13.0, TEXT, 2, 1, &ellipsize(13.0, 1, &value, self.width * 0.55));
            yy += 28.0;
        }
        yy += 8.0;
        yy = self.slider(x, yy, "bgmVolume", t(self.lang, "bgm"), self.snapshot.mixer.background_music, 0.0, 1.0, true, live);
        yy = self.slider(x, yy, "vocalsVolume", t(self.lang, "vocals"), self.snapshot.mixer.original_vocals, 0.0, 1.0, true, live);
        yy = self.slider(x, yy, "monitorVolume", t(self.lang, "monitor"), self.snapshot.mixer.microphone_monitor, 0.0, 1.0, true, live);
        yy = self.slider(x, yy, "microphoneLatency", t(self.lang, "micLat"), self.snapshot.mixer.microphone_latency, -500.0, 1000.0, false, live);
        yy = self.slider(x, yy, "accompanimentLatency", t(self.lang, "accLat"), self.snapshot.mixer.accompaniment_latency, -500.0, 1000.0, false, live);
        yy = self.slider(x, yy, "lyricsLatency", t(self.lang, "lyrLat"), self.snapshot.mixer.lyrics_latency, -500.0, 1000.0, false, live);

        let bw = (self.width - pad * 2.0 - 12.0) / 3.0;
        self.lazer_button(x + pad, yy, bw, 40.0, t(self.lang, "loopA"), Action::LoopA, live && self.controller(), false);
        self.lazer_button(x + pad + bw + 6.0, yy, bw, 40.0, t(self.lang, "loopB"), Action::LoopB, live && self.controller(), false);
        self.lazer_button(x + pad + (bw + 6.0) * 2.0, yy, bw, 40.0, t(self.lang, "clearLoop"), Action::ClearLoop, live && self.controller(), false);
        yy += 48.0;
        self.lazer_button(x + pad, yy, self.width - pad * 2.0, 40.0, t(self.lang, "loopPhrase"), Action::LoopPhrase, live && self.controller(), false);
        yy += 48.0;
        let tw = (self.width - pad * 2.0 - 8.0) * 0.5;
        self.lazer_button(x + pad, yy, tw, 40.0, "OCT", Action::Octave, live && self.controller(), self.snapshot.mixer.octave_fold);
        self.lazer_button(
            x + pad + tw + 8.0,
            yy,
            tw,
        ...
        yy += 56.0;
        self.lazer_button(x + pad, yy, self.width - pad * 2.0, 40.0, self.lang.label(), Action::Language, true, false);
        yy += 52.0;
        fill_rect(x + pad, yy, self.width - pad * 2.0, 56.0, ROW, 8.0);
        fill_text(x + pad + 12.0, yy + 18.0, 13.0, TEXT, 0, 1, t(self.lang, "remember"));
        fill_text(x + pad + 12.0, yy + 38.0, 10.0, MUTED, 0, 0, t(self.lang, "rememberHint"));
        let on = self.remember;
        fill_rect(x + self.width - pad - 52.0, yy + 16.0, 40.0, 22.0, if on { PINK } else { LINE }, 11.0);
        fill_rect(x + self.width - pad - (if on { 22.0 } else { 48.0 }), yy + 18.0, 18.0, 18.0, TEXT, 9.0);
        self.hit(x + pad, yy, self.width - pad * 2.0, 56.0, Action::RememberDevice);
        yy += 68.0;
        self.lazer_button(x + pad, yy, self.width - pad * 2.0, 44.0, t(self.lang, "disconnect"), Action::Disconnect, true, false);
        yy += 36.0;
        fill_text(x + self.width * 0.5, yy, 12.0, MUTED, 1, 0, t(self.lang, "privacy"));
    ...
    fn draw_sheet(
        ...
        let h = (self.height * 0.82).min(640.0);
        let y = self.height - h;
        fill_rect(0.0, y, self.width, h, PANEL, 12.0);
        fill_rect(0.0, y, self.width, 3.0, PINK, 0.0);
        fill_text(20.0, y + 28.0, 18.0, TEXT, 0, 1, &ellipsize(18.0, 1, title, self.width - 110.0));
        fill_text(20.0, y + 50.0, 13.0, MUTED, 0, 0, artist);
        fill_text(self.width - 20.0, y + 28.0, 13.0, MUTED, 2, 0, t(self.lang, "cancel"));
        self.hit(self.width - 90.0, y, 90.0, 48.0, Action::CloseSheet);

        let pad = 14.0;
        let key = format!("{transpose:+}");
        self.stepper(pad, y + 70.0, self.width - pad * 2.0, t(self.lang, "key"), &key, Action::SheetKey(-1), Action::SheetKey(0), Action::SheetKey(1), true);
        ...
        match kind {
            SheetKind::Add => {
                if self.role == "spectator" {
                    self.lazer_button(pad, by, self.width - pad * 2.0, 48.0, t(self.lang, "add"), Action::QueueAdd, true, true);
                } else {
                    let w = (self.width - pad * 2.0 - 12.0) / 3.0;
                    self.lazer_button(pad, by, w, 48.0, t(self.lang, "add"), Action::QueueAdd, true, false);
                    self.lazer_button(pad + w + 6.0, by, w, 48.0, t(self.lang, "addNext"), Action::QueueAddNext, true, false);
                    self.lazer_button(
                        pad + (w + 6.0) * 2.0,
                        by,
                        w,
            ...
            SheetKind::Edit => {
                let w = (self.width - pad * 2.0 - 12.0) / 3.0;
                self.outline_button(pad, by, w, 48.0, t(self.lang, "remove"), Action::QueueRemove(id.into()), self.controller(), DANGER);
                self.lazer_button(pad + w + 6.0, by, w, 48.0, t(self.lang, "playNow"), Action::QueuePlay(id.into()), self.controller(), true);
                self.lazer_button(pad + (w + 6.0) * 2.0, by, w, 48.0, t(self.lang, "save"), Action::QueueAdd, self.controller(), false);
            }
        }
    }
    ...
    fn draw_confirm(&mut self, confirm: Confirm) {
        ...
        self.lazer_button(bx + 16.0, by + 100.0, (box_w - 44.0) * 0.5, 44.0, t(self.lang, "cancel"), Action::ConfirmNo, true, false);
        self.lazer_button(
    ...
    fn stepper(&mut self, x: f32, y: f32, w: f32, label: &str, value: &str, left: Action, reset: Action, right: Action, enabled: bool) {
    ...
    fn outline_button(&mut self, x: f32, y: f32, w: f32, h: f32, label: &str, action: Action, enabled: bool, accent: u32) {
        fill_rect(x, y, w, h, PANEL2, 6.0);
        stroke_rect(x, y, w, h, if enabled { accent } else { LINE }, 6.0, 1.0);
        fill_text(x + w * 0.5, y + h * 0.5, 13.0, if enabled { accent } else { MUTED }, 1, 1, label);
        if enabled && self.controller() {
            self.hit(x, y, w, h, action);
        }
    }

    fn lazer_button(&mut self, x: f32, y: f32, w: f32, h: f32, label: &str, action: Action, enabled: bool, filled: bool) {
        if filled && enabled {
            fill_rect(x, y, w, h, PINK, 6.0);
            fill_text(x + w * 0.5, y + h * 0.5, 13.0, TEXT, 1, 1, label);
        } else {
            fill_rect(x, y, w, h, PANEL2, 6.0);
            stroke_rect(x, y, w, h, if enabled { LINE } else { 0xFF2A2A32 }, 6.0, 1.0);
            fill_text(x + w * 0.5, y + h * 0.5, 13.0, if enabled { TEXT } else { MUTED }, 1, 1, label);
        }
        if enabled && (self.controller() || matches!(action, Action::Language | Action::Disconnect | Action::ConfirmNo | Action::ConfirmYes | Action::CloseSheet | Action::Back | Action::QueueAdd | Action::Page(_))) {
            self.hit(x, y, w, h, action);
        }
    }
...
fn silent_error(error: &str) -> bool {
    matches!(
        error,
        "transition_busy" | "Command rate limit exceeded." | "Sequence was replayed or out of order." | ""
    )
}

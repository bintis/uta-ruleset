...
internal static class UtaStrings
{
    private static readonly Dictionary<string, (string En, string Zh, string Ja)> table = new()
    {
        ...
        ["settings.display"] = ("Display", "显示", "表示"),
        ["settings.playback"] = ("Playback", "播放", "再生"),
        ["settings.microphone"] = ("Microphone", "麦克风", "マイク"),
        ["settings.configure_microphone"] = ("Configure microphone", "配置麦克风", "マイクを設定"),
        ["settings.back"] = ("Back to uta! settings", "返回 uta! 设置", "uta! 設定に戻る"),
        ["settings.lyrics_position"] = ("Lyrics position", "歌词位置", "歌詞の位置"),
        ["settings.lyrics_size"] = ("Lyrics size", "歌词大小", "歌詞サイズ"),
        ["settings.lyrics_font"] = ("Lyrics font", "歌词字体", "歌詞フォント"),
        ["settings.upcoming_lyrics"] = ("Show upcoming lyrics", "显示后续歌词", "次の歌詞を表示"),
        ["settings.lyric_readings"] = ("Show lyric readings", "显示歌词读音", "歌詞の読みを表示"),
        ["settings.pitch_curves"] = ("Pitch curves", "音高曲线", "ピッチ曲線"),
        ["settings.guide_trail"] = ("Singing guide trail", "演唱引导轨迹", "歌唱ガイド軌跡"),
        ["settings.reduced_motion"] = ("Reduced motion", "减少动态效果", "動きを減らす"),
        ["settings.debug_logging"] = ("Debug performance logging", "调试性能日志", "デバッグ性能ログ"),
        ["settings.video"] = ("Show background video", "显示背景视频", "背景動画を表示"),
        ["settings.video_unavailable"] = ("Video controls are unavailable until native video drawable binding is implemented.", "原生视频 drawable 绑定完成前，视频控制不可用。", "ネイティブ動画 drawable の連携が実装されるまで動画コントロールは利用できません。"),
        ["settings.bgm_output"] = ("BGM output", "伴奏输出", "BGM 出力"),
        ["settings.vocals_output"] = ("Vocals output", "人声输出", "ボーカル出力"),
        ["settings.monitor_output"] = ("Microphone monitor output", "麦克风监听输出", "マイクモニター出力"),
        ["settings.background_music"] = ("Background music", "伴奏", "バックグラウンド音楽"),
        ["settings.play_vocals"] = ("Play original vocals", "播放原唱", "オリジナルボーカルを再生"),
        ["settings.original_vocals"] = ("Original vocals", "原唱", "オリジナルボーカル"),
        ["settings.input_gain"] = ("Microphone input gain", "麦克风输入增益", "マイク入力ゲイン"),
        ["settings.sampling_interval"] = ("Pitch sampling interval", "音高采样间隔", "ピッチ分析間隔"),
        ["settings.ear_monitor"] = ("Ear monitor", "耳返监听", "イヤーモニター"),
        ["settings.microphone_latency"] = ("Microphone latency", "麦克风延迟", "マイクレイテンシ"),
        ["settings.auto_measure"] = ("Auto-measure microphone latency", "自动测量麦克风延迟", "マイクレイテンシを自動測定"),
        ["settings.lyrics_opacity"] = ("Lyrics panel opacity", "歌词面板不透明度", "歌詞パネルの不透明度"),
        ["settings.pitch_hud_opacity"] = ("Pitch HUD opacity", "音高 HUD 不透明度", "ピッチ HUD の不透明度"),
        ["settings.safe_area"] = ("HUD safe area padding", "HUD 安全区域边距", "HUD セーフエリア余白"),
        ["settings.video_dim"] = ("Video dim", "视频暗化", "動画の暗さ"),
        ["settings.video_blur"] = ("Video blur", "视频模糊", "動画のぼかし"),
        ["settings.video_offset"] = ("Video offset", "视频偏移", "動画オフセット"),
        ["settings.particle_intensity"] = ("Particle intensity", "粒子强度", "パーティクル強度"),
        ["settings.score_position"] = ("Score HUD position", "分数 HUD 位置", "スコア HUD の位置"),
